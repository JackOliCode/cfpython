...
class Recipe(Base):
  ...
  def return_ingredients_as_list(self):
        #=========
        # by using if not self.ingredients, I am not just checking for empty string but also lists, tuples etc. 
        if not self.ingredients: # should use if not self.ingredients = ''
...
#selected_numbers = list(map(int, ingredient_numbers_split)) --becomes-- int(i) for i in ingredient_numbers_split
            ...
            for number in selected_numbers:
                if 1 <= number <= len(all_ingredients): # if user inputs number outside range this will call an Index Error
                    ...
                    ##=========
                    #feedback - move the IndexError catch to the bottom of function alongside ValueError as we are still within try block
...
#=================
# Request reciepes function for Edit and Delete
# As I am re-using code for both edit and delete functions, better to have a singular function and call this
#=================
def request_recipes():
...
def edit_recipe():
        ...
        recipe_to_edit = request_recipes()
        ...
        if attribute_choice == 1:
            ...
            #=== feedback
            # need to add a try-except error to catch ValueError as I am asking for a type (ask if it's due to it being a type)
